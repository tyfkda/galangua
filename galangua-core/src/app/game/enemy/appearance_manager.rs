            ...
            0 => {
                ...
                if assault_count > 0 {
                    ...
                    for lr in 0..2 {
                        ...
                        let ins = index * 2 + lr;
                        self.orders[ins] = Info::new(0, enemy_type, fi, Vec2I::new(8 * ONE, 0),
                                                     lr == 0, entry.table);
                        index = ((index as i32) + add) as usize;
                    }
                    // Recalc times.
                    for i in 0..self.orders.len() {
                        self.orders[i].time = ((i / 2) * (16 / 3)) as u32;
                    }
                }
            }
            1 => {
                ...
                if assault_count > 0 {
                    ...
                    for i in 0..2 {
                        ...
                        self.orders.insert(index + i, Info::new(time, enemy_type, fi, Vec2I::new(8 * ONE, 0),
                                                                entry.flip_x, entry.table));
                    }
                    // Recalc times.
                    for i in 0..self.orders.len() {
                        self.orders[i].time = (i * (16 / 3)) as u32;
                    }
                }
            }
            2 => {
                ...
                if assault_count > 0 {
                    ...
                    for i in 0..2 {
                        ...
                        assault_index += 1;
                        let enemy_type = enemy_types[((index + i) & 1) as usize];
                        self.orders.insert(index + i, Info::new(time, enemy_type, fi, ZERO_VEC, entry.flip_x,
                                                                entry.table));
                    }
                    // Recalc times.
                    for i in 0..self.orders.len() {
            ...
            3 => {
                ...
                if assault_count > 0 {
                    ...
                    for lr in 0..2 {
                        ...
                        let ins = index * 2 + lr;
                        self.orders[ins] = Info::new(time, enemy_type, fi, Vec2I::new(8 * ONE * flag, 0),
                                                     entry.flip_x, entry.table);
